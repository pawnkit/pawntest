...
type Config struct {
	...
	Profile             bool            `json:"profile"              yaml:"profile"              toml:"profile"`
	ProfileOutput       string          `json:"profile_output"       yaml:"profile_output"       toml:"profile_output"`
	...
	NativePlugin        string          `json:"native_plugin"        yaml:"native_plugin"        toml:"native_plugin"`
	PluginArchitecture  string          `json:"plugin_architecture"  yaml:"plugin_architecture"  toml:"plugin_architecture"`
	PluginWorker32      string          `json:"plugin_worker_32"     yaml:"plugin_worker_32"      toml:"plugin_worker_32"`
	PluginWorker64      string          `json:"plugin_worker_64"     yaml:"plugin_worker_64"      toml:"plugin_worker_64"`
	present             map[string]bool `json:"-" yaml:"-" toml:"-"`
}
...
func (cfg *Config) merge(override Config) {
	...
	for key, value := range values {
		if override.present[key] {
			merged[key] = value

			continue
		}

		if len(override.present) > 0 {
			continue
		}
...
func (a *TestCmd) applyRuntimeConfig(cfg Config) {
	if cfg.Profile {
		a.Profile = true
	}

	if a.ProfileOutput == "profile.json" && cfg.ProfileOutput != "" {
		a.ProfileOutput = cfg.ProfileOutput
	}

	if a.NativePlugin == "" {
		a.NativePlugin = cfg.NativePlugin
	}

	if a.PluginArchitecture == "x86" && cfg.PluginArchitecture != "" {
		a.PluginArchitecture = cfg.PluginArchitecture
	}

	if a.PluginWorker32 == "" {
		a.PluginWorker32 = cfg.PluginWorker32
	}

	if a.PluginWorker64 == "" {
		a.PluginWorker64 = cfg.PluginWorker64
	}
}
